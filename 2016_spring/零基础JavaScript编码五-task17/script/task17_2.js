 ...
 * 代码是抄别人的，
 * 原作者：hansneil
 * 原项目地址：https://github.com/hansneil/hansneil.github.io/tree/master/baidu_ife/2-17
 * 待完善
 */
/* 数据格式演示
 var aqiSourceData = {
...
/**
 * 数据所在位置
 *其实就是基于图表视口的总width，按照把柱状图的一个块状和左边的空白当成一个整体来看。块状和空白宽度是一样的，所以
 *1.每个块状的宽度就是 width / (总的数据量x2) 就是posObj.width
 *2.每个块状加空白的宽度就是 width / (总的数据量) 就是posObj.left
 *3.posObj.offsetLeft其实就是空白宽度的一半，代码里写的太复杂了＝ ＝
 */
...
function renderChart() {
    ...
    //key为某日的日期
        ...
        innerHTML += "<div class='aqi-hint' style='bottom: " + (selectedData[key] + 10) + "px; left:" + getHintLfeft(posObj, i++) + "px'>" + key + "<br/> 空气质量指数: " + selectedData[key] + "</div>"
    }
...
function initCitySelector() {
  ...
  // 读取aqiSourceData中的城市，获取城市名称
    var cityArr = Object.getOwnPropertyNames(aqiSourceData);
    //然后设置id为city-select的下拉列表中的选项
